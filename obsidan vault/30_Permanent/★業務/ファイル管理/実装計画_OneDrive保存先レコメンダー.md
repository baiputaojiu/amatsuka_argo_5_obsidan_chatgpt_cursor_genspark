...
- 実パスを出力・保存せず、Step 1でスキップした少数のフォルダに未収録の子フォルダがないか一度だけ確認する

完了条件：

- 年度ルートの同一・双方の入れ子・Windowsの大文字小文字差を単体テストで拒否できる
- スキップした少数のフォルダが候補の欠落を生じさせていないことを読み取り専用で確認できる
...
| 要件定義§20 | 主な確認先 | 方法 | Step 6結果 |
|---|---|---|---|
| 機能1 | `test_catalog.py`、実環境走査 | 自動＋手動 | 合格 |
| 機能2 | `test_terms.py`、`test_msg_reader.py` | 自動＋Windows結合 | 合格 |
| 機能3 | `test_ranking.py` | 自動 | 合格 |
| 機能4 | 判定コア、GUI | 自動＋手動 | 合格 |
| 機能5 | 判定コア、GUI | 自動＋手動 | 合格 |
| 機能6 | `test_ranking.py` | 自動 | 合格 |
| 機能7 | `test_ranking.py` | 自動 | 合格 |
| 機能8 | GUI、クリップボード、一時設定 | 自動＋手動 | 合格 |
| 機能9 | `test_codex_prompt.py`、GUI | 自動＋手動 | 合格 |
| 機能10 | `test_audit.py` | 自動 | 合格 |
| 安全性1 | 実環境走査前後 | 手動 | 合格 |
| 安全性2 | ネットワーク制限環境、依存・ソース確認 | 自動＋構造確認 | 合格 |
| 安全性3 | `test_audit.py` | 自動 | 合格 |
| 安全性4 | `test_ranking.py` | 自動 | 合格 |
| 安全性5 | `test_catalog.py` | 自動 | 合格 |
| 安全性6 | `test_terms.py`、一時領域確認 | 自動 | 合格 |
...
- 今年度・昨年度の読み取り専用走査は、除外後の収録件数を非公開とし、スキップなし
- 走査時間は未温約3.5秒、温約0.3秒

Step 1実装後の再確認（2026-07-31）：

- 現行のフォルダ構成では、除外後の収録件数は非公開で、個別列挙できなかったフォルダは少数
...
- 個別列挙できなかった少数の対象は、いずれも深い末端フォルダだった
...
- 非公開の実カタログに`20XX0101_定例資料_サンプル.pptx`相当の3語を適用し、複数候補、今年度の`（Output）定例成果物`が1位、昨年度候補も残ることを読み取り専用で確認した。判定時間は約206ミリ秒だった

Step 2.1実装後の確認（2026-08-01）：

- 非公開の実カタログの判定準備は約117.6ミリ秒で、カタログまたは設定変更時だけ実行する
...
- ジャンクション除外後の実フォルダ走査は1回目3.624秒、2回目0.886秒で、収録件数は非公開、少数のフォルダをスキップした
...
- 単体テスト87件、WindowsのGUI統合テスト1件、実MSGのOutlook COM結合テスト1件、Ruffの静的検査が成功した
- 実ランタイムの非公開カタログを読み込み、手動検索`定例 サンプル`で複数候補を表示し、起動時不備通知0件であることをパス非表示で確認した
- レビュー後、カタログ更新時も投入直後の候補0件フラグを保持し、検索語変更時は前回の確定パス表示をクリアするよう修正した

Step 6受け入れ確認（2026-08-01）：

- 単体テスト87件、Windows GUI・実MSG・合成MSGの結合テスト3件、Ruffの静的検査が成功した
- 起動時の`settings.json`不在・不正、今年度・昨年度・保存先未定・カタログ不在の6条件を、一時領域だけを使うGUI結合テストで確認した
- 実MSGは件名・本文・添付一覧へのアクセスだけを確認し、合成MSGでは本文整形、`image`＋数字の除外、通常添付名の採用を確認した。実業務データはリポジトリへ保存していない
- 実環境走査の収録件数は非公開とし、列挙できない深い末端フォルダを少数スキップして他の走査を継続した。対象に年度ルートや主要な保存先フォルダは含まれない
- ファイル未投入の手動検索で複数候補を表示し、確定パスをエクスプローラと「名前を付けて保存」の双方へ貼り付けて対象フォルダへ移動できた。保存操作はキャンセルした
- ネットワーク制限環境で設定・実カタログを読み込み、手動検索、候補算出、Codex相談用プロンプト生成まで成功した。実行時依存とソースにネットワーククライアントはない
- GUI手動確認では本番Auditを使わず一時ファイルへ記録し、確認後に削除した
- 対象OneDriveの確認前後フィンガープリントを比較し、ファイル・フォルダの件数、相対パス、種別、サイズ、更新日時に変化がないことを確認した
- 要件定義書§20の機能10件・安全性6件はすべて合格した
