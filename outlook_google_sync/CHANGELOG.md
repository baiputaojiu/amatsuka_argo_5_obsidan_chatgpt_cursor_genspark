...
- **GUI**: セットアップ完了時（`credentials.json` と `token.json` あり）は起動後に接続テストを自動実行。手動「接続テスト」ボタンは維持。未セットアップ時はチェックリストのみ表示し自動テストしない。
- **GUI**: ログ欄のエラー行（接続テスト失敗・`ERROR:` 等）を赤文字で表示。

## 0.3.4 - 2026-07-15

- **GUI**: 起動時の日付を標準期間（今日−1ヶ月〜今日＋2ヶ月）に自動設定。メイン画面に「標準期間」ボタンを追加。
- **GUI**: `credentials.json` / `token.json` 未整備時に初回セットアップ・チェックリストダイアログを表示（認証・設定へ誘導。「後で」でスキップ可）。

## 0.3.3 - 2026-07-14

- **`scripts/run_gui.bat`**: 起動前に `outlook_google_sync/` へ `cd`。`.venv` 未作成時はグローバル Python へフォールバックせずエラー表示。異常終了時は終了コードを表示して `pause`。
- **GUI**: `FocusSafeDateEntry` の表示形式を `yyyy/mm/dd` に固定（起動経路によるロケール差で `M/d/yy` になる問題を解消）。
