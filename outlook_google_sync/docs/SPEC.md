...
- 同期実行・プレビュー・重複修復・設定変更は GUI で行う。接続テストは **起動時に自動実行**（セットアップ完了時）し、手動ボタンでも再実行できる。
...
### 接続テスト（Ch24）
- **トリガー**: (1) GUI 起動時・セットアップ完了時に自動（`worker(connection_test)`）(2) 「接続テスト」ボタンによる手動再実行
- セットアップ未完（`credentials.json` / `token.json` 欠如）のときは自動実行しない（チェックリストのみ）
- 疎通と最小読取の確認。同期成功は保証しない（CT-01〜05）
- Google: 2段構成（基本接続 + 対象カレンダー確認）
- カレンダー未選択時: 成功扱い + 「未実施」メッセージ（CT-CAL-01〜03）
- ICS: 11項目の詳細チェック（Ch25）
- 結果はログ欄に出力（成功時に messagebox は出さない）。失敗行は赤文字で表示する
